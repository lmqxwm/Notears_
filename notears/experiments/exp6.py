...
def estimate_once_notears(d, sem_type, graph_type, seed=1, lambda1=0, loss_type='l2', BB=100, noise=None):
    ...
    if noise == "normal":
        ...
        noise_scale = np.random.uniform(1, 3, d)
    else:
        noise_scale = None
    ...
    w_est, *losses = linear.notears_linear(X, W_true=W_true, lambda1=0, loss_type=loss_type)
    results[:5, 1] = losses
    print(losses)
    results[5, 1] = np.linalg.norm(w_est-W_true, ord='fro')
    for i in range(B):
        random.shuffle(inds)
        w_est, *losses = linear.notears_linear(X[:, inds], W_true=W_true, lambda1=0, loss_type=loss_type)
        w_est_re = restore_from_per(inds, w_est)
        print(losses)
        results[:5, i+2] = losses
        results[5, i+2] = np.linalg.norm(w_est_re-W_true, ord='fro')
        
    results = pd.DataFrame(results.T, columns=[
    "loss_est", "loss_l1", "obj_aug", "obj_dual", "h", "fro_dist"])
    
    results.to_csv("./results_loss3/result_"+str(seed)+"_"+sem_type+"_"+str(d)+".csv", index=False)


def estimate_template_notears(sem_type, seed=1, lambda1=0, loss_type='l2', BB=100, noise=None):
    print("=============processing seed=", seed, "================")
    d = 12
    #utils.set_random_seed(seed)
    n = 2 * d
        ...
        for a2 in permutations([3,4,5]):
            for a3 in permutations([6,7,8]):
                for a4 in permutations([9,10,11]):
                    ...
                    w_est, results[:5, count+1] = model.fit(X[:, new_ind], lambda1=0)
                    w_est_re = restore_from_per(inds, w_est)
                    results[5, count+1] = np.linalg.norm(w_est_re-W_true, ord='fro')
...
def estimate_template_dagma(sem_type, seed=1, lambda1=0, loss_type='l2', BB=100, noise=None):
    print("=============processing seed=", seed, "================")
    d = 12
    #utils.set_random_seed(seed)
    n = 2 * d
    ...
    inds = [i for i in range(d)]
    w_est, results[0, 1] = model.fit(X, lambda1=0)
    results[1, 1] = np.linalg.norm(w_est-W_true, ord='fro')
        ...
        for a2 in permutations([3,4,5]):
            for a3 in permutations([6,7,8]):
                for a4 in permutations([9,10,11]):
                    count += 1
                    new_ind = list(a1) + list(a2) + list(a3) + list(a4)
                    w_est, results[0, count+1] = model.fit(X[:, new_ind], lambda1=0)
                    w_est_re = restore_from_per(inds, w_est)
                    results[1, count+1] = np.linalg.norm(w_est_re-W_true, ord='fro')
...
def estimate_once_dagma(d, sem_type, graph_type, seed=1, lambda1=0, loss_type='l2', BB=100, noise=None):
    print("=============processing seed=", seed, "================")
    #utils.set_random_seed(seed)
    n = d-1
    s0 = n
    ...
    #utils.set_random_seed(seed+d)
    inds = [i for i in range(d)]
    w_est, l2_loss = model.fit(X, lambda1=0)
    results[0, 1] = l2_loss
    results[1, 1] = np.linalg.norm(w_est-W_true, ord='fro')
    for i in range(B):
        random.shuffle(inds)
        w_est, l2_loss = model.fit(X[:, inds], lambda1=0)
        results[0, i+2] = l2_loss
        w_est_re = restore_from_per(inds, w_est)
        results[1, i+2] = np.linalg.norm(w_est_re-W_true, ord='fro')
...
if __name__ == '__main__':
    ...
    with pool:
        for sm in range(len(semtypes)):
            ...
            #   ds)
            pool.map(partial(estimate_once_notears, 
              graph_type="BP", seed=5050, sem_type=semtypes[sm], lambda1=0, BB=200, noise="normal"),
              ds)
            pool.map(partial(estimate_once_notears, 
              graph_type="BP", seed=5051, sem_type=semtypes[sm], lambda1=0, BB=200, noise="uni"), 
              ds)
        ds = [5, 10, 15, 20]
        pool.map(partial(estimate_once_notears, 
              graph_type="ER", sem_type="logistic", seed=5060, lambda1=0, loss_type='logistic'),
              ds)
        pool.map(partial(estimate_once_notears, 
              graph_type="ER", sem_type="logistic", seed=5061, lambda1=0, loss_type='poisson'),
              ds)
