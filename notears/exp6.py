...
def estimate_once(d, graph_type, sem_type, seed=1, lambda1=0, loss_type='l2', BB=100):
    ...
    X = utils.simulate_linear_sem(W_true, n, sem_type, noise_scale=np.random.uniform(0.5, 5, d))
        ...
        for a2 in permutations([3,4,5]):
            for a3 in permutations([6,7,8]):
                for a4 in permutations([9,10,11]):
                    ...
                    results[:, count+1] = linear.notears_linear(X[:, new_ind], W_true=W_true, lambda1=0, loss_type="l2")
...
def estimate_once_dagma(d, graph_type, sem_type, seed=1, lambda1=0, loss_type='l2', BB=100):
    ...
    model = linear2.DagmaLinear(loss_type='l2')
    

    B = np.min([math.factorial(d), BB])
    results = np.zeros(B+2)
    results[0] = 0.5 / X.shape[0] * ((X-X@W_true) ** 2).sum()
    #all_perm = list(permutations(range(X.shape[1])))
    #utils.set_random_seed(seed+d)
    inds = [i for i in range(d)]
...
if __name__ == '__main__':
    ...
    with pool:
        ...
        #       graph_type="ER", seed=5022, sem_type=semtypes[sm], lambda1=0), 
        #       ds)
